...
;layui.define(["laytpl", "laypage", "layer", "form", "util"], function (e) {
    ...
    F.prototype.config = {
        ...
        var e = this;
        e.fullSize(), e.scrollPatch(), e.setColsWidth()
        ...
        var c = this, s = c.config, u = e[s.response.dataName] || [], h = [], p = [], v = [], m = function () {
            ...
            }), c.layBody.scrollTop(0), c.layMain.find("." + f).remove(), c.layMain.find("tbody").html(h.join("")), c.layFixLeft.find("tbody").html(p.join("")), c.layFixRight.find("tbody").html(v.join("")), c.renderForm(), "number" == typeof e && c.setThisRowChecked(e), c.syncCheckAll(), c.scrollPatch(), l.close(c.tipsIndex), s.HAS_SET_COLS_PATCH || c.setColsPatch(), void(s.HAS_SET_COLS_PATCH = !0))
        };
        ...
        a.elem.on("click", 'input[name="layTableCheckbox"]+', function () {
            ...
            }), a.syncCheckAll(), a.renderForm("checkbox")) : (a.setCheckData(l, n), a.syncCheckAll()), layui.event.call(this, u, "checkbox(" + p + ")", v.call(this, {
                checked: n,
                type: o ? "all" : "one"
            }))
        ...
        a.layBody.on("change", "." + N, function () {
            ...
            if (!e.data("off") && i) {
                var l = t('<input class="layui-input ' + N + '">');
                return l[0].value = e.data("content") || a.text(), e.find("." + N)[0] || e.append(l), void l.focus()
            }
